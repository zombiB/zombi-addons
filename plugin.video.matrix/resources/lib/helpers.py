...
from resources.lib import unjuice, unjuice2
...
def sort_sources_list(sources):
    if len(sources) > 1:
        try:
            ...
            VSlog(r'Scrape sources sort failed |int(re.sub("\D", "", x[0])|')
            try:
                sources.sort(key=lambda x: re.sub("[^a-zA-Z]", "", x[0].lower()))
            except:
                VSlog('Scrape sources sort failed |re.sub("[^a-zA-Z]", "", x[0].lower())|')
    return sources
...
def scrape_sources(html, result_blacklist=None, scheme='http', patterns=None, generic_patterns=True):
    ...
    def __parse_to_list(_html, regex):
        ...
        if matches:
            VSlog('Scrape sources |%s| found |%s|' % (regex, matches))
        return matches
...
def get_media_url(
    ...
    response = net.get(url, headers=headers, allow_redirects=True)
...
def pick_source(sources, auto_pick=None):
    ...
    if len(sources) == 1:
        ...
        if auto_pick:
            ...
            result = xbmcgui.Dialog().select(('choose_the_link'), [str(source[0]) if source[0] else 'Unknown' for source in sources])
            if result == -1:
                raise VSlog(('no_link_selected'))
            else:
                return sources[result][1]
    else:
        raise VSlog(('no_video_link'))
...
def base164(e):
    t = '???D?FGHIJKL?NOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.,~'
    n = ''
    o = 0
    while o < len(e):
        r = t.index(e[o])
