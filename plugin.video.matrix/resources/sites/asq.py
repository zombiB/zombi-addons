...
URL_MAIN = 'https://www.3asq.tv/'



ANIM_NEWS = ('https://www.3asq.tv/forumdisplay.php?f=348', 'showMovies')
...
# ([^<]+) .+?
    sPattern = '<a href="([^<]+)" onmouseover="var img=document.+?<strong>([^<]+)</strong></a>'
...
def showLive():
    ...
    sHtmlContent = sHtmlContent.decode("windows-1256").encode("utf-8")
    # (.+?) ([^<]+) .+?
    sPattern = '<a href="([^<]+)" id="thread_title_.+?>([^<]+)</a>.+?<span class="smallfont"'
...
def showHosters():
    ...
    #sHtmlContent = sHtmlContent.replace('<iframe src="//www.facebook.com/plugins/like.php','')
         ...
         # (.+?) ([^<]+) .+?  
    sPattern = '<br />([^<]+)<br />.+?<a href="([^<]+)" target="_blank">GoogleDrive</a> '
    oParser = cParser()
    aResult = oParser.parse(sHtmlContent, sPattern)
