...
SITE_IDENTIFIER = 'halqat'
SITE_NAME = 'halqat'
SITE_DESC = 'arabic vod'
 
URL_MAIN = 'http://halqat.online/'


SERIE_TR = ('http://halqat.online/video/category.php?cat=turkish-series', 'showMovies')
...
def showLive():
    ...
    sPattern = '<a href="([^<]+)" title="([^<]+)">.+?<div class="pm-video-labels hidden-xs">.+?</div><img src="([^<]+)" alt'
    ...
    if (aResult[0] == True):
        ...
        progress_.VSclose(progress_)
    # (.+?)
    sPattern = '<a href="(.+?)" target="(.+?)" rel="nofollow">'
