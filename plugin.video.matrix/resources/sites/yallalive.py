...
# ([^<]+) .+?
    ...
    sPattern = '<li><a href="([^<]+)">.+?<img src="http://www.yalla-shoot.com/images/upload/images/.+?.png" width="30" /><br>([^<]+)</td>.+?<img src="http://www.yalla-shoot.com/images/upload/images/.+?.png" width="30" /><br>([^<]+)</td>'
...
def showHosters():
    ...
    sPattern = '<br><font color=.+?>(.+?)</font>.+?src="(.+?)"' 
    aResult = oParser.parse(sHtmlContent, sPattern)
    if (aResult[0] == True):
        total = len(aResult[1])
        progress_ = progress().VScreate(SITE_NAME)
