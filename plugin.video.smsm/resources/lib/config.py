...
class cConfig():
    ...
    def __init__(self):
        ...
        if (self.__bIsDharma):
            ...
            self.__sFileDB = os.path.join(self.__oCache,'smsm.db')
    ...
    def createDialogOK(self, label):
        ...
        oDialog.ok('smsm', label)  
        return oDialog
        
    def createDialogYesNo(self, label):
        oDialog = xbmcgui.Dialog()
        qst = oDialog.yesno("smsm", label)
        return qst
    ...
    def finishDialog(self, dialog):
        if xbmcgui.Window(10101).getProperty('search') != 'true':
            dialog.close()
            xbmc.log('\t[PLUGIN] smsm: close dialog')
            del dialog
            return False
    ...
    def error(self, e):
        ...
        xbmc.log('\t[PLUGIN] smsm Erreur: '+str(e))
        #cConfig().ERROR.append(e)

    def log(self, e):
        xbmc.log('\t[PLUGIN] smsm: '+str(e))
    ...
    def openerror(self):
        ...
        self.win.getControl(1).setLabel("smsm popup Erreur")
    ...
    def WindowsBoxes(self, sTitle, sFileName, num,year = ''):
        ...
        if (extendedinfo):
            if num == "2":
                self.showInfo('smsm', 'Lancement de ExtendInfo')
