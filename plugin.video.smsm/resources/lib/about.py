...
class cAbout:       
    ...
    def getRootPath(self, folder):
        sMath = cConfig().getAddonPath().replace('plugin.video.smsm', '') 
    ...
    def checkupdate(self):
            ...
            #dialog = cConfig().showInfo("smsm", "Cherche les mises a jour")
            ...
            if result:
                ...
                if (sDown != 0):
                    ...
                    cConfig().setSetting('service_time', str(datetime.datetime.now()))
                    dialog = cConfig().showInfo("smsm", "Mise à jour disponible")   
                else:
                    #cConfig().showInfo('smsm', 'Fichier a jour')
                    cConfig().setSetting('service_time', str(datetime.datetime.now()))
    ...
    def checkdownload(self):
            ...
            if result: 
                ...
                #self.TextBoxes('smsm mise à Jour', sContent)
                cConfig().setSetting('service_time', str(datetime.datetime.now()))
                cConfig().setSetting('home_update', str('false'))
