...
SITE_IDENTIFIER = 'tv_alnoor'
SITE_NAME = 'tv-alnoor'
SITE_DESC = 'arabic vod'

URL_MAIN = 'http://www.tv-alnoor.com/tvshows'

SERIE_TR = ('http://www.tv-alnoor.com/tvshows/tvshow/1', 'showMovies')


URL_SEARCH = ('http://www.tv-alnoor.com/search?keyword=', 'showMovies')
...
def showSearch():
    ...
    if (sSearchText != False):
        sUrl = 'http://www.tv-alnoor.com/search?keyword='+sSearchText
        showMovies(sUrl)
        oGui.setEndOfDirectory()
        return
...
def showMovies(sSearch = ''):
    ...
    sPattern = '<div class="article">.+?<a href="(.+?)" class="mheight">.+?<div style="background-image: url((.+?))" >.+?</div>.+?</a>.+?<div class="details">.+?<p>(.+?)</p>.+?</div>'
    ...
    if (aResult[0] == True):
        ...
        for aEntry in aResult[1]:
            ...
            oGui.addMovie(SITE_IDENTIFIER, 'showSeries', sTitle, '', sThumbnail, '', oOutputParameterHandler)
...
def showSeries():
    ...
    sPattern = '<a href="([^<]+)" class="video-link".+?style="background-image:url(([^<]+))">(.+?)</a>'
    ...
    if (aResult[0] == True):
        ...
        for aEntry in aResult[1]:
            ...
            if 'seasons' in sUrl:
                oGui.addMisc(SITE_IDENTIFIER, 'showSeries', sTitle, '', sThumbnail, '', oOutputParameterHandler)
            else:
...
def __checkForNextPage(sHtmlContent):
    sPattern = '</span></a></li><li><a href="(.+?)">.+?</a>'
