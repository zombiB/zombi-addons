...
class cClear:
    ...
    def main(self, env):
        ...
        if (env == 'urlresolver'):
            ...
            try:
                sUrl = 'https://raw.githubusercontent.com/zombiB/zombi-addons/master/plugin.video.bein/changelog.txt'
                oRequest =  urllib2.Request(sUrl)
                oResponse = urllib2.urlopen(oRequest)
                sContent = oResponse.read()
                from about import cAbout
                cAbout().TextBoxes('bein Changelog', sContent)
            except:            
            ...
            if dialog.yesno('bein', 'Are you sure ?','','','No', 'Yes'):
